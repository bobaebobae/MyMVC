...
<head>
<meta charset="UTF-8">
<title>02postMethodTest.jsp 페이지</title>
...
<body>
	...
	<%-- <form name="memberFrm" action="/MyMVC/postMethodTest2.do" method="post"> --%>
	<%-- 컨텍스트명(context name)을 알아오는 것이 <%= request.getContextPath()%> 이다. 
		  즉, <%= request.getContextPath()%>이 /MyMVC으로 나온다. --%>
	
	<form name="memberFrm" action="<%= ctxPath%>/postMethodTest2.do" method="post">  <!-- get을 하면 @override get 방식이 쓰임 -->
		
			<%-- submit 을 하면 <%= ctxPath%>/postMethodTest2.do 로 보내어서 처리를 요청한다.
			     /MyMVC/postMethodTest2.do 은 누가 처리를 해주는지 배치서술자인 web.xml 에 기술되어져 있어야 한다.
			     web.xml 에 가보면 <servlet-mapping>에 URL 패턴으로 /postMethodTest2.do 이 기술되어져 있고,
		...
		<fieldset>
			<legend>개인성향 테스트(POST method)</legend>			<!-- 주소창의 ?를 기준으로 URL?Data // 돌려보면 name의 데이터 값이 다 나옴.(get 경우) -->
			<ul>
				<li>
