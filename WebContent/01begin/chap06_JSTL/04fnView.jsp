...
<body>
	...
	<table>
		...
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 길이</td>
			<td>${ fn:length(subject) }</td>
			<td>int</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 대문자로</td>
			<td>${ fn:toUpperCase(subject) }</td>
			<td>String</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 소문자로</td>
			<td>${ fn:toLowerCase(subject) }</td>
			<td>String</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 에서 JSP만 추출</td>
			<td>${ fn:substring(subject, 5, 8) }</td>
			<td>String</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 에서 JSP 다음의 문자열을 추출</td>
			<td>${ fn:substringAfter(subject, "JSP") }</td>  <!-- 또는 "JSP" 대신에 'JSP' 를 사용해도 됨.  -->
			<td>String</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 에서 JSP 이전의 문자열을 추출</td>
		...
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 에서 S 를 에스 로 변환</td>
			<td>${ fn:replace(subject, "S","에스") }</td>  <!-- 대소문자 구분함  -->
			<td>String</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 에서 JSP 가 위치한 인덱스를 구함</td>
			<td>${ fn:indexOf(subject, "JSP") }</td>  <!-- 대소문자 구분함  -->
			<td>int</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 이 JSTL 로 시작합니까?</td>
			<td>${ fn:startsWith(subject, "JSTL") }</td>  <!-- 대소문자 구분함  -->
			<td>boolean</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 이 ary) 로 끝납니까?</td>
			<td>${ fn:endsWith(subject, "ary)") }</td>  <!-- 대소문자 구분함  -->
			<td>boolean</td>
		</tr>
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 속에 Tag 라는 문자열이 포함되어 있습니까?</td>
		...
		<tr>
			<td>문자열 &quot;JSTL(JSP Standard Tag Library)&quot; 을 공백을 구분자로 배열로 만들기</td>
			<td><c:set var="array" value="${ fn:split(subject, ' ') }" /></td>  <!-- 대소문자 구분하지 않음 -->
			<td>String[]</td>
		</tr>
		<tr>
			<td>공백을 구분자로 만들어진 배열 array 를  하나의 문자열로 만들기(공백을 제거한 결과)</td>
			<td>${ fn:join(array, '') }</td> 
			<td>String</td>
		</tr>
		<tr>
			<td>공백을 구분자로 만들어진 배열 array 를  하나의 문자열로 만들기(구분자를 콤마로 한 것)</td>
			<td>${ fn:join(array, ',') }</td> 
			<td>String</td>
		</tr>	
	</table>
