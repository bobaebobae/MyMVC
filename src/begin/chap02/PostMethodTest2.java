...
public class PostMethodTest2 extends HttpServlet { // HttpServlet  에 정의되어있는 접근제한자는  protected /public 이라고 되야한다....
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 	// get방식이라 doGet함   // requet는 입력도구/ response는 출력도구
		...
		System.out.println(">>>확인용 : doGet(request, response) 메소드  호출됨");
		   ...
		   if(foodArr != null) {	//null이 아닐 때만 실행해라! 입력을 안하고 넘기면 nullpointExeption 나옴
			   for(int i=0; i<foodArr.length; i++) {
				   System.out.println("foodArr["+i+"] => " + foodArr[i]); // 넘어온 음식을 찍어보겠다.
			   }
		   }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		throws IOException {	// post방식이라 doPost함
